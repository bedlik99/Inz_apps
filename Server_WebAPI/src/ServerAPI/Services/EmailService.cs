...
namespace ServerAPI.Services
{
    public class EmailService : IEmailRepo
    {
        ...
        public string SendEmail(IEnumerable<RegisteredLabUserDTO> 
            setOfEmailData, string labName)
        {
            ...
            foreach (var user in setOfEmailData)
            {
                ...
                MailboxAddress emailFrom = new MailboxAddress
                    (_emailSettings.Name, _emailSettings.EmailId);
                emailMessage.From.Add(emailFrom);
                ...
                BodyBuilder emailBodyBuilder = new BodyBuilder();
                emailBodyBuilder.TextBody = $"Dane użytkownika do " +
                    $"logowania się na laboratorium: "
                    + $"\n Username: {user.Email}"
            ...
            try
            {
                SmtpClient emailClient = new SmtpClient();
                emailClient.CheckCertificateRevocation = false;
                emailClient.Connect(_emailSettings.Host, 
                    _emailSettings.Port, SecureSocketOptions.StartTls);
                emailClient.Authenticate(_emailSettings.EmailId, 
                ...
                emailClient.Disconnect(true);
                emailClient.Dispose();
                return $"Operacja zakończyła się sukcesem. " +
                    $"Poświadczenia zostały wysłane prawidłowo.";
            }
            catch (Exception ex)
            {
                foreach (var item in setOfEmailData)
                {
                    var user = _context
                    .RegisteredUserItems
                    .SingleOrDefault(u => u.Email == item.Email && 
                    u.UniqueCode == item.UniqueCode);
        ...
        public string SendEmailDbData(RegisteredUser user)
        {
            try
            {
                MimeMessage emailMessage = new MimeMessage();
